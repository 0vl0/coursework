...
void GLDisplayWidget::drawTetrahedron(){
    _geomWorld.drawTetrahedron();
}

void GLDisplayWidget::drawPyramid(){
    _geomWorld.drawPyramid();
}

void GLDisplayWidget::drawSquare(){
    _geomWorld.drawBoundingBox();
}

void GLDisplayWidget::drawQueen(){
    _geomWorld.drawQueen();
}

void GLDisplayWidget::drawTriangulation(){
    _geomWorld.drawTriangulation();
}

void GLDisplayWidget::reset(){
    _geomWorld.reset();
}

void GLDisplayWidget::drawNeighbors(){
    _geomWorld.drawNeighbors();
}

void GLDisplayWidget::computeLaplacian(){
    for (int i = 0; i<100; i++){
    _geomWorld.temperature();
    updateGL();
    }
}

void GLDisplayWidget::addPointTriangulation(){
    _geomWorld.addPointTriangulation();
}
